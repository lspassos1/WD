...
The server exposes 33 tools. Most are cache-reads over pre-seeded Redis keys (sub-second). Six (`get_world_brief`, `get_country_brief`, `analyze_situation`, `generate_forecasts`, `search_flights`, `search_flight_prices_by_date`) call live LLMs or external APIs and are slower.
...
| `get_country_macro` | IMF WEO bundle plus World Bank/BIS/FATF financial exposure: inflation, current account, GDP/capita, unemployment, savings-investment gap, external debt, banking exposure, AML/CFT listing risk. |
| `get_country_resilience` | Country resilience ranking, static inputs, recovery-capacity seeds, and energy-system resilience seeds. |
