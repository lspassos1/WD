...
export class DeckGLMap {
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    this.state = {
      ...
      layers: normalizeExclusiveChoropleths(initialState.layers),
    };
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'resilience-choropleth-layer': {
        ...
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(resilienceName)}</strong><br/>Resilience: <span style="color:${levelColor};font-weight:600">${resilienceEntry.overallScore.toFixed(1)}/100</span><br/><span style="text-transform:capitalize;opacity:.7">${text(formatResilienceChoroplethLevel(resilienceEntry.level))}</span></div>`,
        };
      }
  ...
  public setLayers(layers: MapLayers): void {
    ...
    this.state.layers = normalizeExclusiveChoropleths(layers);
