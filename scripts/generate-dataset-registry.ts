...
function formatStringMap(entries: Array<[string, string]>): string {
  ...
  return `{\n${entries
    .map(([name, value]) => `  ${JSON.stringify(name)}: ${renderKeyExpression(value)},`)
    .join('\n')}\n}`;
}
...
function emit(): void {
  ...
  writeFileSync(
    ...
    [
      '/* AUTO-GENERATED by scripts/generate-dataset-registry.ts. DO NOT EDIT. */',
      `export const HEALTH_BOOTSTRAP_KEYS = ${formatStringMap(healthBootstrapKeys)};`,
      `export const HEALTH_STANDALONE_KEYS = ${formatStringMap(healthStandaloneKeys)};`,
